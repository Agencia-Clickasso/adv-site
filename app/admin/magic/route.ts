  ...
  try {
    ...
    if (!payload) {
      return NextResponse.redirect(
        new URL("/admin/login?error=Link expirado ou inválido", request.url)
      )
    }

    const issued = await issueAdminSession(payload.email)
    const response = NextResponse.redirect(
      new URL("/admin/posts?success=Login realizado", request.url)
    )
    ...
    return NextResponse.redirect(
      new URL("/admin/login?error=Não foi possível validar o link", request.url)
    )
  }
}
