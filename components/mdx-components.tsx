...
const components: MDXComponents = {
  ...
  h2: ({ children }: { children: ReactNode }) => (
    <h2 className={`${headingClass} mt-12 mb-4 text-3xl leading-tight text-slate-950 sm:text-4xl`}>
      {children}
    </h2>
  ),
  h3: ({ children }: { children: ReactNode }) => (
    <h3 className={`${headingClass} mt-10 mb-3 text-2xl leading-tight text-slate-900 sm:text-3xl`}>
      {children}
    </h3>
  ),
  h4: ({ children }: { children: ReactNode }) => (
    <h4 className={`${headingClass} mt-8 mb-2 text-xl leading-tight text-slate-900 sm:text-2xl`}>
      {children}
    </h4>
  ),
  p: ({ children }: { children: ReactNode }) => (
    <p className="mb-5 text-[1.06rem] leading-8 text-slate-700">{children}</p>
  ),
  ul: ({ children }: { children: ReactNode }) => (
    <ul className="mb-6 ml-5 list-disc space-y-3 text-[1.02rem] leading-8 text-slate-700 marker:text-[#9c744a]">
      {children}
    </ul>
  ),
  ol: ({ children }: { children: ReactNode }) => (
    <ol className="mb-6 ml-5 list-decimal space-y-3 text-[1.02rem] leading-8 text-slate-700 marker:text-[#9c744a]">
      {children}
    </ol>
  ),
