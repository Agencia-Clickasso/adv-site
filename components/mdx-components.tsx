...
const components: MDXComponents = {
  ...
  h3: ({ children }: { children: ReactNode }) => (
    <h3 className={`${headingClass} mt-9 mb-3 text-2xl leading-tight text-slate-900 sm:text-[1.65rem]`}>
      {children}
    </h3>
  ),
  h4: ({ children }: { children: ReactNode }) => (
    <h4 className={`${headingClass} mt-7 mb-2 text-xl leading-tight text-slate-900 sm:text-2xl`}>{children}</h4>
  ),
